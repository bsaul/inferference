...
test_that("retrieval functions work", {
  ...
  # Indirect effects
  expect_equivalent(as.numeric(as.matrix(ie(test, .35, .4))), 
                    c(0.0666863333283656, 0.0679513395569612, 
                      -0.0664958449045302, 0.199868511561261))
  
  # Total effects
  expect_equivalent(as.numeric(as.matrix(te(test, .35, .4))), 
                    c(0.313421628689255  , 0.14769062385725, 
                      0.0239533250747932, 0.602889932303717))
  
  # Overall effects
  expect_equivalent(as.numeric(as.matrix(oe(test, .35, .4))), 
